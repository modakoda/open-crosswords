...
// ~500KB per its Zod schema) with headroom for the request envelope.
// `importSchema.text`'s own `.max()` remains the hard limit either way. Not
// using oRPC's built-in `BodyLimitPlugin` here: as of @orpc/server 1.15.0 it
// breaks under Next.js 16's Turbopack production build with "Cannot read
// private member #state from an object whose class did not declare it" — a
// bundler/library incompatibility, not a config mistake; revisit switching
// back once that's fixed upstream.
