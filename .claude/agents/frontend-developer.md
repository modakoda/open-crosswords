...
- Auth-aware pages rely on better-auth session state via `src/lib/auth-client.ts`; every `/admin/dashboard/*` view is gated server-side by `getAdmin()` (session + `ADMIN_EMAILS`) with a redirect in its own `page.tsx` as well as in the shared `src/app/admin/dashboard/layout.tsx` — the layout alone is not a boundary, so a new view route needs its own guard (`/admin` redirects to `/admin/dashboard`, which redirects on to the first view), and `/client/dashboard` is gated by `getCurrentUser()` (any signed-in user). Check how existing pages read the session before adding a new gated view.
...
- Client components call the API via the oRPC browser client (`src/lib/orpc/client.ts`, `orpc.<router>.<procedure>(...)`) — not raw `fetch`. Match the existing patterns in `src/components/GenerateForm.tsx` and `src/components/admin/**`.
- Visitor- and client-facing pages (generate, solve, print, sign-up, `/client/**` — not `/admin/**`) are translated via `src/lib/i18n/` (`en`/`lt` dictionaries). Server pages resolve a `Messages` object with `getMessages(locale)` and pass it down as a prop; don't hardcode a new user-visible string in these components — add a key to both `src/lib/i18n/messages/en.ts` and `lt.ts` instead.
