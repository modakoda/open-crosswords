...
- Auth-aware pages rely on better-auth session state via `src/lib/auth-client.ts`; `/admin/dashboard` is gated server-side by `getAdmin()` (session + `ADMIN_EMAILS`) with a redirect (`/admin` itself just redirects to `/admin/dashboard`), and `/client/dashboard` is gated by `getCurrentUser()` (any signed-in user). Check how existing pages read the session before adding a new gated view.
