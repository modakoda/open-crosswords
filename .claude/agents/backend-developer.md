...
- Business logic lives in `src/lib/` (crossword engine in `src/lib/crossword/**`, `puzzles/` (types+queries), `entries.ts`, `taxonomy.ts` (languages + language-scoped categories), `import.ts`, `ai/draft.ts`, `solve-state.ts`); oRPC procedures stay thin and call into it.
