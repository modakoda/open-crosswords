...
- Business logic in `src/lib/`: the crossword engine (`src/lib/crossword/**`), plus `puzzles/` (types+queries), `entries.ts`, `taxonomy.ts` (languages + language-scoped categories), `import.ts`, `ai/draft.ts`, `solve-state.ts`. Procedures stay thin.
