...
- Self-hosting is supported alongside Vercel: `docker-compose.yml` (local Postgres) and a `Tiltfile` for local dev. If a `Dockerfile` / `output: 'standalone'` exists, don't remove it to "simplify" for Vercel. Defer Dockerfile / image / Compose work to the `docker` skill.
